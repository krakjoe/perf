/*
  ...
  +----------------------------------------------------------------------+
 */
#ifndef HAVE_PERF_MONITOR
#define HAVE_PERF_MONITOR
...
php_perf_monitor_t* php_perf_monitor_create(void) {
	...
	php_perf_monitor_t *monitor = 
		(php_perf_monitor_t*) 
			calloc(1, sizeof(php_perf_monitor_t));
...
int php_perf_monitor_lock(php_perf_monitor_t *monitor) {
	return pthread_mutex_lock(&monitor->mutex);
}

int32_t php_perf_monitor_check(php_perf_monitor_t *monitor, int32_t state) {
	return (monitor->state & (state));
}

int php_perf_monitor_unlock(php_perf_monitor_t *monitor) {
	return pthread_mutex_unlock(&monitor->mutex);
}

int32_t php_perf_monitor_wait(php_perf_monitor_t *monitor, int32_t state) {
...
int32_t php_perf_monitor_wait_locked(php_perf_monitor_t *monitor, int32_t state) {
	int32_t changed = FAILURE;
	int      rc      = SUCCESS;
...
int32_t php_perf_monitor_waiting(php_perf_monitor_t *monitor) {
	return pthread_cond_wait(&monitor->condition, &monitor->mutex);
}

int32_t php_perf_monitor_broadcast(php_perf_monitor_t *monitor) {
...
void php_perf_monitor_unset(php_perf_monitor_t *monitor, int32_t state) {
	monitor->state &= ~state;

	pthread_cond_signal(&monitor->condition);
}

void php_perf_monitor_destroy(php_perf_monitor_t *monitor) {
